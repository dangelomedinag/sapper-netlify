...
{#if popover}
  <div
    class="absolute bg-black bg-opacity-75 p-10 w-full h-full z-50 -translate-y-1/2 -translate-x-1/2 flex justify-center items-center">
    <p
...
<nav class="bg-white fixed top-0 shadow-lg w-full z-10">
          ...
          <div class="flex items-center ml-3 ">
            {#each options as option}
              <buttom
                ...
                class="cursor-pointer px-3 py-1 ml-2 self-center text-sm font-medium text-gray-800 antialiased rounded-md focus:outline-none hover:bg-blue-100 hover:text-blue-500 focus:bg-blue-100">
                {option.mode}
              </buttom>
            {/each}
            ...
            <!-- <a
              href="/"
              class="px-3 py-1 ml-2 self-center text-sm font-medium text-gray-800 antialiased rounded-md focus:outline-none hover:bg-blue-100 hover:text-blue-500 focus:bg-blue-100">Categorias</a>
...
<div class="relative">
  <aside class="h-full bg-neutral w-56 fixed z-0 top-0 left-0 pt-56">
    <div class="w-56 fixed top-0 left-0 pt-16 bg-gray-400 h-56">
      <!-- <select bind:value={selected}>
        {#each options as option}
          <option value={option}>{option.mode}</option>
        {/each}
      </select> -->
    </div>
    <hr />
    <nav class="text-center overflow-x-hidden h-full max-h-full">
      {#each productos as producto (producto.id)}
        <div
          ...
          class="p-3 bg-neutral cursor-pointer hover:bg-gray-300">
  ...
  <!-- <main class="ml-56 px-10 pt-24">
    {#if current}
      <code class="inline-block w-full">id: {current.id}</code>
      <code class="inline-block w-full">categoria:
        {categoria_parse(current.categoria_id)}</code>
      <code class="inline-block w-full">categoria_id:
        {current.categoria_id}</code>
      <h1>{current.nombre}</h1>
      <hr />
      <p>{current.descripcion}</p>
      <p>{current.tipo}</p>
      <input type="number" value={current.precio} disabled />
      <input type="number" value={current.descuento} disabled />
      <input
        type="checkbox"
